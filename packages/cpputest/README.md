...
`%check` enables the complete upstream CMake test gate and runs its 77 test
groups serially. Grouped mode is upstream's default and still executes every
self-test; serial execution isolates the deliberate signal/crash checks under
RISC-V user-mode emulation. The installed smoke test compiles and runs a real
CppUTest test against the installed library.
