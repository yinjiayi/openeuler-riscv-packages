...
the exact built tree to container-local temporary storage, grants execute-only
traversal on the `mktemp` parent and read/traversal on only that copy, verifies
both libtool wrappers as `bin`, and runs the complete parallel upstream suite
there.
