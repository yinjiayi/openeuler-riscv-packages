...
Downstream release `3` matches the pinned archive's exact `qt-session-lock-2.1.0` source root, loads `Qt6::WaylandClientPrivate` through openEuler Qt 6.5's public `WaylandClient` component, and keeps the private target and all Wayland session-lock functionality enabled. It declares the matching Qt base private headers, Qt Wayland development files, XKB development provider, and Wayland protocol dependencies. The build uses one explicit out-of-source directory and verifies that the upstream `sessionlock-test` executable is produced. The source SHA-256 remains unchanged.
