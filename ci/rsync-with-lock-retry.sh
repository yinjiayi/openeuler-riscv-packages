...
jitter_max=${RRSYNC_LOCK_JITTER_MAX_SECONDS:-2}
jitter_seed=${RRSYNC_LOCK_JITTER_SEED:-0}
...
[[ $jitter_seed =~ ^[0-9]+$ ]] \
  || { printf 'invalid RRSYNC_LOCK_JITTER_SEED: %s\n' "$jitter_seed" >&2; exit 2; }
[[ ${1:-} == -- && $# -ge 2 ]] \
...
while :; do
  ...
  jitter=$(( (jitter_seed + attempt) % (jitter_max + 1) ))
