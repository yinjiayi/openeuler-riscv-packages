...
- A **bounded DNF transaction** is a dependency or installed-RPM transaction
  ...
  repository, accept unresolved dependencies, share a mutable per-run cache
  across jobs, or turn a failed transaction into success.
- A **locked-image metadata refresh** is an OCI build that starts from the exact
  public, anonymously verified digest in `ci/image.lock`, revalidates its live
  RPM database against the embedded manifest, replaces the repository tooling,
  and hydrates only target-DNF metadata. It does not reinstall the rootfs or
  claim a new bootstrap transaction. Changes to bootstrap, portable RPM-database
  finalization, manifest semantics, or build configuration select the separate
  `full-bootstrap` target instead.
- An **immutable target DNF metadata cache** is the official openEuler
  `primary`/`filelists` repository state hydrated once by the openEuler 24.03
  LTS SP3 `riscv64` DNF inside the digest-bound CI image, after the target RPM
  database has been finalized. It contains no RPM payloads and is verified by
  loading the official repository with DNF cache-only while the container has
  no network. Package jobs may reuse this image baseline, but still resolve all
  dependencies from the enabled official and per-run supplemental repositories,
  download required RPM payloads through their normal network phase, and retain
  official RPM signature checking. This is immutable image input, not a shared
  writable runner cache or permission to accept expired partial downloads.
...
The build image tag is `ghcr.io/yinjiayi/openeuler-riscv64-rpmbuild:24.03-lts-sp3-rva23`. A full-bootstrap root filesystem must be constructed only from:

```text
https://repo.openeuler.org/openEuler-24.03-LTS-SP3/everything/riscv64/rva23/riscv64/
```

Package CI reads an immutable digest from `ci/image.lock`; a mutable tag is never accepted as build evidence. Routine cache work uses the locked-image metadata-refresh target, records the exact parent digest in both the OCI label and build evidence, verifies the live RPM database and checksum-bound manifest before deleting only `/var/cache/dnf`, and never executes the bootstrap installer or RPM-database importer. A manual request or a change to bootstrap, finalization, manifest, or build-configuration inputs selects `full-bootstrap`. Image publication records the `repomd.xml` digest, installed RPM manifest, OCI digest, QEMU version, selected build mode, resolved locked-base reference, and RVA23 probe result; the parent-image label applies to metadata refreshes because a full bootstrap has no image parent. Large full-bootstrap metadata and signing-key RPM downloads use a **resumable authenticated download**: finite HTTPS Range attempts retain only the current partial file, and the completed bytes are not exposed at their final path until their SHA-256 matches the checksum authenticated by the fetched `repomd.xml`. Resume does not weaken GPG checking or accept partial data. The full-bootstrap payload transaction uses a **cached single-stream download**: DNF downloads one RPM at a time with finite per-transfer retries, while two bounded whole-transaction attempts share completed RPM downloads only inside the disposable build root. The second attempt reuses the identical approved repository, key, package set, and install root; the complete gpgchecked transaction must still succeed. This reduces HTTP/2 contention without changing the approved repository or lowering the low-speed threshold. After either the portable RPM database is finalized or the locked parent database is revalidated, the target openEuler DNF starts from an explicitly cleaned cache and performs one official-repository-only `makecache` transaction with the same long `7300`-second image-build budget and `4200,3000`-second attempts. The resulting immutable target DNF metadata cache is retained in the image without RPM payloads. Target verification then loads the official repository from that cache with `--cacheonly` under Docker network mode `none`; every package transaction still keeps `gpgcheck=1`, resolves dependencies normally, and downloads required RPMs from the enabled repositories. The installed RPM manifest is a sorted, checksum-bound record of each package's identity, architecture, and RPM-defined SHA-1/SHA-256 immutable-header digests. Every row and digest is format-validated before use. These digests bind original package metadata across database implementations; they do not replace the gpgchecked DNF transaction or claim a second payload verification.
...
A **bot-PR required-check bridge** is the fail-closed protected-main dispatch used only when `github-actions[bot]` creates a PR and GitHub suppresses that PR's ordinary workflow event. The bridge binds the open PR's exact head, current base, same-repository branch, bot identity, complete allowlisted file set, and a unique parent-run nonce. Its infrastructure allowlist contains only `infra/ci-image-<12 hex>` atomically modifying the existing `ci/image.lock` and `ops/actions-runner-fleet/cleanup-image.lock`, or `catalog/discovery-<timestamp>-<run id>` adding the identically named immutable snapshot. The cleanup lock is an operational mirror of the immutable CI image reference, not an independent image choice; the bridge verifies that both candidate locks and both base locks resolve to the same image/digest pair. Package CI itself is dispatched from `main`, never from the bot branch; the run title binds the PR number, candidate head, and nonce while the workflow head must equal the supplied protected-base SHA. Infrastructure mode exercises no package build or RPM publication, and each PR number has an independent concurrency key. Only after the exact Package CI run and all six package-policy jobs have a terminal `success`, and the live PR remains open, unmerged, exact-head/base bound, and Auto-merge-disabled, does this bridge publish the six package-policy contexts. Any orchestration or status-write failure overwrites all six with `error`. The separate protected-main `configure` check bridge creates a GitHub CheckRun rather than a commit status, only for an exact bot-created digest-lock PR whose suppressed Auto Merge Policy run, two immutable lock contents, and current PR lease are fully attested; ordinary PRs continue to obtain `configure` directly from Auto Merge Policy. Catalog snapshot PRs remain fail-closed at `configure` until an equivalent catalog-content attestation is implemented.
